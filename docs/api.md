# API JSON do Ponto — Referência da API

> **FONTE CANÔNICA** desta doc (a API é definida AQUI, no servidor). Os
> consumidores — `ponto-cli` e `ponto-extension` — mantêm cópias sincronizadas
> em `docs/api.md`; ao mudar um endpoint JSON, atualizar ESTA e replicar nos
> dois (não há geração automática).

> Fontes no código: `app/models/access_token.rb`, `app/controllers/concerns/
